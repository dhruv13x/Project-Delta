...
def get_credentials():
    ...
    This version is self-contained and does not depend on the global bot object.
    """
    # --- DEFINITIVE FIX: Use a local logger ---
    logger = logging.getLogger("credentials_loader")

    source_name = ""
    if 'google.colab' in sys.modules:
        from google.colab import userdata
        logger.info("Colab environment detected. Loading secrets.")
        source_name = "Colab Secrets"
        source = userdata.get
    else:
        from dotenv import load_dotenv
        load_dotenv()
        logger.info("Local environment detected. Loading secrets from .env file.")
    ...
    if missing_keys:
        logger.critical(f"CRITICAL: Missing required secrets in {source_name}: {missing_keys}")
        return None
